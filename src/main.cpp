...
#if defined(NDEBUG)
# error "Pranacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x77a82c817eb1620c98a9c3c673cde2ed25310a5bccd8ece33018318ea0563d1e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pranacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pranacoin Signed Message:\n";
...
{
    // Pranacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pranacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pranacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1504006813, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 22 Aug 2017 Yesterday the US went dark";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040c102d34b8b1e40125c93bd968bdb71caeae6e29f806c434fdce2c4dc673e03bc79ab6e1934d883094bc602910a0d40cdb5ae3b19b4f7ee0c3957bae6070684d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1504006813;
        ...
        assert(block.hashMerkleRoot == uint256("0x468f8cd8c2b8b20447f40374ed2a975d07330128c070ed92f4831488d517ad9c"));
...
unsigned char pchMessageStart[4] = { 0x2d, 0xf6, 0xf2, 0x2a }; // Pranacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PranacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PranacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PranacoinMiner(CWallet *pwallet)
{
    printf("PranacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pranacoin-miner");
    ...
    try { loop {
        ...
        printf("Running PranacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PranacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PranacoinMiner, pwallet));
}
